# 定向简历改写 · Tailor Your Resume to Any JD
...
`resume-tailor-generic` 把「按 JD 改写」做成可重复流程：
...
`resume-tailor-generic` turns JD tailoring into a repeatable pipeline:
...
**推荐搭配 · Best paired with:** [resume-database-generic](../resume-database-generic/) — 先归档全部事实与 STAR，再按岗定向改写。没有素材库，每次改写都在重新翻 PDF；有库，筛选关用的是 **对的弹药**。  
Build your experience database first, then tailor per application — the screen only sees what you can surface fast.
...
| 范围 Scope | 路径 Path |
|------------|-----------|
| 项目级 Project | `<your-project>/.cursor/skills/resume-tailor-generic/` |
| 个人级 User | `~/.cursor/skills/resume-tailor-generic/`（Windows: `%USERPROFILE%\.cursor\skills\`） |

确认存在 `SKILL.md`，在对话中说：**「按 resume-tailor-generic 根据这份 JD 改简历并导出一页 PDF。」**  
Ensure `SKILL.md` exists, then prompt: **"Use resume-tailor-generic to tailor my resume to this JD and export a one-page PDF."**

### 2. 准备输入 · Prepare inputs

- **目标 JD** — 文本 / 链接 / 截图
- **原始简历** 或 **`简历数据库.md`**（来自 [resume-database-generic](../resume-database-generic/)）
- **可选** — 岗位名、城市、证件照路径
...
python path/to/resume-tailor-generic/scripts/check_resume_skill.py 示例公司+张三+示例岗位.md --markdown-only

# 导出一页 PDF · Export one-page PDF (--preset tight | fill)
python path/to/resume-tailor-generic/scripts/markdown_resume_to_pdf.py 示例公司+张三+示例岗位.md --preset fill

# 证件照裁剪 · Crop profile photo (default 540×790)
python path/to/resume-tailor-generic/scripts/crop_profile_photo.py photo.jpg -o profile-photo-cropped.png
```

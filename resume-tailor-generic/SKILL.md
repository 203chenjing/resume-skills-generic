...
description: 根据原始简历与目标 JD 改写中文定向简历（Markdown/一页 PDF），输出 STAR 编号条目、量化结果与关键词匹配；不绑定特定候选人项目规则。在用户要求优化简历、按 JD 定制、导出 PDF、或写编号式经历时使用。建简历数据库见 resume-database-generic 技能。
---
...
知识库缺失时，按 [resume-database-generic](../resume-database-generic/SKILL.md) 判断是否先建库。
